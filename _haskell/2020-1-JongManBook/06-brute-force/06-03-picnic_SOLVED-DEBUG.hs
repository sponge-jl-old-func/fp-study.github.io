...
main :: IO()
main = do 
    print $ debug_countPairings input
    where
...
debug_countPairings taken = do
    ...
    case firstFree of
      ...
      otherwise -> do
          ...
          -- | 그 다음 친구(B)부터 마지막 친구(n)까지 짝을 맺을 수 있는지 탐색(lambda do 구문)하여 리스트에 담음
          rets <- forM [firstFree'+1..n] $ \ pairWith -> do
            -- | <DEBUG> 현재 짝을 맺을 친구와 대상 친구들의 탐색 단계 출력
            traceM $ "(firstFree', pairWith) : " ++ "(" ++ show firstFree' ++  ", " ++ show pairWith ++ ")"
            ...
            case (not $ taken!!pairWith) && (areFriends!!firstFree'!!pairWith) of
              ...
              True -> do
                ...
                -- | 짝맺기를 성공 후 재귀호출한 뒤 결과를 반환
                ret
          -- | <DEBUG> 위 설명을 반복한 결과 리스트 출력
          traceM $ "rets: " ++ (show rets)
          -- | 결과 리스트를 sum으로 매핑해서 모두 합산
